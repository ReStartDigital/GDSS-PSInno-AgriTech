...
-- Enums
CREATE TYPE user_role AS ENUM ('farmer', 'buyer', 'transporter', 'agent',
'admin');
CREATE TYPE listing_status AS ENUM ('active', 'sold', 'cancelled');
CREATE TYPE order_status AS ENUM (
 'pending', 'negotiating', 'confirmed', 'packed',
 'in_transit', 'delivered', 'cancelled'
);
CREATE TYPE payment_status AS ENUM ('pending', 'success', 'failed', 'refunded');
CREATE TYPE payment_channel AS ENUM ('mobile_money', 'card');
CREATE TYPE mobile_money_provider AS ENUM ('mtn', 'vodafone', 'airteltigo');
CREATE TYPE transport_status AS ENUM (
 'open', 'accepted', 'en_route', 'picked_up', 'in_transit', 'delivered',
'cancelled'
);
...
CREATE TABLE users (
 ...
 paystack_email VARCHAR(255) NOT NULL UNIQUE, -- auto-generated on
registration
 pin_hash VARCHAR(255), -- bcrypt hash, nullable
until PIN set
 profile_photo_url TEXT,
 location GEOMETRY(Point, 4326), -- PostGIS point (lng,
lat)
 agent_id UUID REFERENCES users(id) ON DELETE SET NULL,
 is_active BOOLEAN NOT NULL DEFAULT TRUE,
 average_rating NUMERIC(3, 2) DEFAULT 0.00, -- denormalised, updated
on new rating
...
CREATE INDEX idx_users_location ON users USING GIST(location); -- spatial index
...
CREATE TABLE packaging_options (
 ...
 suitable_for TEXT[] NOT NULL, -- e.g. ARRAY['tomatoes',
'peppers']
...
CREATE TABLE produce_listings (
 id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
 farmer_id UUID NOT NULL REFERENCES users(id) ON DELETE
RESTRICT,
 vegetable_type VARCHAR(100) NOT NULL,
 quantity_kg NUMERIC(10, 2) NOT NULL CHECK (quantity_kg > 0),
 price_per_kg_ghs NUMERIC(10, 2) NOT NULL CHECK (price_per_kg_ghs >
0),
 harvest_date DATE NOT NULL,
 images JSONB NOT NULL DEFAULT '[]', -- Cloudinary URL
array
 recommended_packaging_id UUID REFERENCES packaging_options(id) ON DELETE SET
NULL,
...
CREATE TABLE orders (
 id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
 buyer_id UUID NOT NULL REFERENCES users(id) ON DELETE
RESTRICT,
 farmer_id UUID NOT NULL REFERENCES users(id) ON DELETE
RESTRICT,
 listing_id UUID NOT NULL REFERENCES produce_listings(id) ON
DELETE RESTRICT,
 quantity_kg NUMERIC(10, 2) NOT NULL CHECK (quantity_kg > 0),
 price_per_kg_ghs NUMERIC(10, 2) NOT NULL, -- locked at order
time
 negotiated_price_per_kg_ghs NUMERIC(10, 2), -- set if
negotiation occurred
 produce_subtotal_ghs NUMERIC(12, 2) NOT NULL,
 packaging_type_id UUID REFERENCES packaging_options(id) ON DELETE SET
NULL,
 ...
 delivery_location GEOMETRY(Point, 4326), -- buyer delivery
GPS
...
CREATE TABLE payments (
 id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
 order_id UUID NOT NULL UNIQUE REFERENCES orders(id) ON DELETE
RESTRICT,
 ...
 paystack_response JSONB, -- full Paystack
response stored
...
CREATE TABLE transport_requests (
 ...
 order_id UUID NOT NULL UNIQUE REFERENCES orders(id) ON DELETE
RESTRICT,
 transporter_id UUID REFERENCES users(id) ON DELETE SET NULL, -- NULL
until accepted
 pickup_location GEOMETRY(Point, 4326) NOT NULL,
 dropoff_location GEOMETRY(Point, 4326) NOT NULL,
 distance_km NUMERIC(8, 2), -- calculated by
PostGIS
 estimated_cost_ghs NUMERIC(10, 2),
 packaging_type_name VARCHAR(255), -- denormalised for
transporter briefing
...
CREATE INDEX idx_transport_pickup ON transport_requests USING
