...
export const AUTH_CONSTANTS = {
  OTP: {
    ...
    RESEND_COOLDOWN_SECONDS:
      Number(process.env.AUTH_OTP_COOLDOWN_SECONDS) || 60, // Prevent spamming resend buttons
  },
  REGISTRATION_TOKEN: {
  ...
  REFRESH_TOKEN: {
    ...
    EXPIRY_SECONDS:
      Number(process.env.AUTH_REFRESH_TOKEN_EXPIRY_SECONDS) || 7 * 24 * 60 * 60, // Default: 7 days
  },
  PIN: {
  ...
  RATE_LIMIT: {
    AUTH_WINDOW_MS:
      Number(process.env.AUTH_RATE_LIMIT_WINDOW_MS) || 15 * 60 * 1000, // Default: 15 minutes
    AUTH_MAX_REQUESTS: Number(process.env.AUTH_RATE_LIMIT_MAX_REQUESTS) || 5, // Strictly blocks Arkesel gateway abuse
    GENERAL_WINDOW_MS:
      Number(process.env.RATE_LIMIT_GEN_WINDOW_MS) || 15 * 60 * 1000,
    GENERAL_MAX_REQUESTS:
      Number(process.env.RATE_LIMIT_GEN_MAX_REQUESTS) || 100,
  },
  UNVERIFIED_REGISTRATION_TTL_HOURS:
    Number(process.env.AUTH_UNVERIFIED_USER_TTL_HOURS) || 24, // Threshold for the database cleanup cron job
} as const;
