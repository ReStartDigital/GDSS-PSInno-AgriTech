...
import type { ProduceListingEntity } from "../../database/entities/ProduceListing.js";
...
export class ListingsService {
  ...
  async create(
    ...
    if (caller.role === UserRole.AGENT) {
      ...
      throw new ForbiddenException(
        "Only farmers and agents can create listings.",
      );
    }

    // Validate packaging option exists if provided
    if (dto.recommended_packaging_id) {
      const pkg = await this.repo.findPackagingById(
        dto.recommended_packaging_id,
      );
      if (!pkg) {
        throw new NotFoundException(
          "Packaging option not found or is no longer active",
        );
      }
    }
  ...
  async update(
    ...
    const priceOrQtyChanged =
      dto.quantity_kg !== undefined || dto.price_per_kg_ghs !== undefined;
    ...
    if (
      dto.recommended_packaging_id !== undefined &&
      dto.recommended_packaging_id !== null
    ) {
      const pkg = await this.repo.findPackagingById(
        dto.recommended_packaging_id,
      );
    ...
    const willSupportDelivery =
      dto.supports_delivery ?? listing.supportsDelivery;
  ...
  async remove(
    ...
    await this.repo.update(listingId, {
      status: "cancelled" as ProduceListingEntity["status"],
    });
  ...
  async uploadImage(
    ...
    if (caller.role !== UserRole.FARMER && caller.role !== UserRole.AGENT) {
      throw new ForbiddenException(
        "Only farmers and agents can upload listing images.",
      );
    }
    ...
    logger.debug("Listing image uploaded", {
  ...
  async recommendPackaging(dto: PackagingRecommendDto): Promise<{
    ...
    const recommendations = options.map((opt) => {
      const suitableForArray =
        opt.suitableFor || (opt as any).suitable_for || [];
  ...
  private async assertCallerOwnsListing(
    ...
    throw new ForbiddenException(
      "Only farmers and agents can modify listings.",
    );
  }
   ...
   */
  private async assertAgentOwnsClient(
    agentId: string,
    farmerId: string,
  ): Promise<void> {
    const assignment = await userRepository.findAssignmentByAgentAndUser(
      agentId,
      farmerId,
    );
    if (!assignment) {
