...
export const createListingSchema = z
  .object({
    ...
    recommended_packaging_id: z
      .string()
      .uuid("recommended_packaging_id must be a valid UUID")
      .optional(),
  ...
  .superRefine((data, ctx) => {
    ...
    if (!validFulfilment) {
      ctx.addIssue({
        ...
        message:
          "At least one of supports_delivery or supports_pickup must be true",
        path: ["supports_pickup"],
      });
    }

    if (
      data.auto_confirm_until_kg !== undefined &&
      data.auto_confirm_until_kg > data.quantity_kg
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Auto-confirm limit cannot exceed the total quantity available",
...
export const updateListingSchema = z
  ...
  .refine(
    ...
    {
      message: "At least one fulfilment mode must remain enabled.",
      path: ["supports_pickup"],
    },
  );
...
export const listingsQuerySchema = z
  ...
  .refine(
    ...
    {
      message:
        "Both lat and lng must be provided together for location-based filtering",
      path: ["lat"],
    },
  )
  .refine(
    ...
    {
      message: "min_price_kg must be less than or equal to max_price_kg",
      path: ["min_price_kg"],
    },
  );
