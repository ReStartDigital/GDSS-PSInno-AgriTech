...
class ArkeselClient {
  ...
  /**
   * Sends a plain SMS via Vegelink's standard send endpoint. Used for
  ...
  async sendSms(phone: string, message: string): Promise<ArkeselSendResult> {
    try {
      ...
      if (!ok) {
        logger.warn("Vegelink SMS send returned non-success status", {
          phone,
      ...
      logError("Vegelink sendSms request failed", error, { phone });
      return { success: false, errorReason: "NETWORK_ERROR" };
    }
  }

  /**
   * Generates and sends an OTP via Vegelink's dedicated OTP endpoint.
    ...
    try {
      const response = await this.http.post("/otp/generate", {
        ...
        message:
          "Hello " +
          `${full_name ?? "there!"}` +
          ",\nThis is OTP from Vegelink, %otp_code%",
        sender_id: process.env.ARKESEL_SENDER_ID || "VegeLink",
        type: "numeric",
      });

      // Vegelink returns { code: "2000", message: "Successful" } on success.
      const ok = response.data?.code === "1000";
      if (!ok) {
        logger.warn("Vegelink generateOtp rejected the request", {
          phone,
          response: response.data,
        });
        return {
      ...
      logError("Vegelink generateOtp request failed", error, { phone });
      // Network failure, timeout, or 4xx/5xx from Vegelink — treat uniformly
  ...
  /**
   * Verifies an OTP previously generated via Vegelink's own OTP system.
   * NOTE: VegeLink's primary OTP flow stores its own hashed OTP in Redis
   * (see otp.repository.ts) rather than relying solely on Vegelink's verify
  ...
  async verifyOtp(phone: string, code: string): Promise<ArkeselSendResult> {
    try {
      ...
      logError("Vegelink verifyOtp request failed", error, { phone });
      return { success: false, errorReason: "NETWORK_ERROR" };
    }
  }
