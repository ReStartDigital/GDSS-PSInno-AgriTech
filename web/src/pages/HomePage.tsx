...
const ROLES = [
  { id: 'farmer',      label: 'Farmer',      icon: 'leaf'     as const, bg: 'rgba(214,255,205,0.35)', border: '#e5e7eb', actions: ['List produce', 'Confirm orders via SMS', 'Receive MoMo payment'] },
  { id: 'buyer',       label: 'Buyer',       icon: 'shopping' as const, bg: 'rgba(214,255,205,0.2)',  border: '#e5e7eb', actions: ['Browse marketplace', 'Place & pay orders', 'Track delivery live'] },
  { id: 'transporter', label: 'Transporter', icon: 'truck'    as const, bg: 'rgba(143,188,143,0.2)',  border: '#e5e7eb', actions: ['View nearby jobs', 'Accept deliveries', 'Update live status'] },
  { id: 'agent',       label: 'Agent',       icon: 'user'     as const, bg: 'rgba(38,65,35,0.06)',   border: '#e5e7eb', actions: ['Register farmers', 'Manage listings', 'Confirm orders on behalf'] },
]
...
export default function HomePage() {
  ...
  return (
    <div className="page-stack">
      ...
      {/* ── Authenticated hero (logged in) ─────────────────────────────── */}
      {role ? (
        <AuthHero role={role} firstName={firstName} />
      ) : (
        <GuestHero />
      )}
      ...
      {/* ── Fresh produce preview (unauthenticated) ───────────────────── */}
      {!role && <FreshProducePreview />}
        ...
        <div className="overview-mockup">
          ...
          <div className="mockup-body">
            <div className="mockup-row">
              ...
              <span style={{ marginLeft: 'auto', color: '#264123', fontWeight: 700 }}>GH₵ 4.50/kg</span>
            </div>
            <div className="mockup-row">
...
function AuthHero({ role, firstName }: { role: string; firstName: string | null }) {
  const greeting = getGreeting()

  const quickActions: { label: string; icon: Parameters<typeof Icon>[0]['name']; to: string; bg: string; color: string }[] = [
  ...
  return (
    <section className="auth-hero-banner">
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ margin: 0, color: 'rgba(214,255,205,0.8)', fontSize: '0.95rem' }}>
          Welcome back to VegeLink Ghana
        </p>
      </div>
    ...
    <section className="section-card">
      <div className="section-heading">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(249,115,22,0.1)', display: 'grid', placeItems: 'center' }}>
            <Icon name="pulse" />
          </div>
          <div>
            <p className="eyebrow" style={{ color: '#6b7280' }}>Live prices</p>
      ...
      <div className="pulse-grid">
        {MARKET_PULSE.map((item) => (
          <div key={item.crop} className="pulse-card" style={{ borderColor: item.accent + '22', background: item.tint }}>
            ...
            <span className={`pulse-change ${item.change.startsWith('+') ? 'up' : 'down'}`}>
              {item.change}
            </span>
          </div>
        ))}
  ...
  return (
    <section className="section-card">
      <div className="section-heading">
        ...
        <Link to="/marketplace" className="secondary-button" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', minHeight: 40, padding: '0 16px', fontSize: '0.85rem' }}>
          See all →
        </Link>
      </div>
      ...
      <div className="produce-card-grid">
        {listings.map((listing, i) => (
          <div key={listing.id} className="produce-card">
            <div className="produce-card-visual" style={{ background: TINTS[i % TINTS.length] }}>
              <ProduceMark accent={ACCENTS[i % ACCENTS.length]} />
            </div>
            <div className="produce-card-body">
              <p className="produce-card-name">{listing.vegetable_type}</p>
              <p className="produce-card-price">
                GH₵{listing.price_per_kg_ghs}
                <span className="produce-card-unit"> /kg</span>
              </p>
              {listing.farmer && (
                <p className="produce-card-farmer">{listing.farmer.firstName}</p>
              )}
            </div>
          </div>
        ))}
...
function ProduceMark({ accent }: { accent: string }) {
  return (
    <div style={{ width: 52, height: 52, borderRadius: '50%', background: accent, boxShadow: `0 4px 12px ${accent}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <section className="section-card">
      ...
      <div className="dashboard-grid">
        {(['farmer', 'buyer', 'transporter', 'agent'].includes(role)) && (
        ...
        {role === 'agent' && (
          <div className="dashboard-panel">
            ...
            <p style={{ color: '#374151', fontSize: '0.9rem', margin: '0 0 16px', lineHeight: '1.5' }}>
              Onboard and manage farmer clients, view their active produce, and create listings on their behalf.
            </p>
        ...
        {role === 'transporter' && (
          <div className="dashboard-panel">
            ...
            <p style={{ color: '#374151', fontSize: '0.9rem', margin: '0 0 16px' }}>
              Find available delivery jobs near you and manage your active routes.
            </p>
